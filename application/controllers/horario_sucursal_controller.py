# CONTROLLER_PY_LOGGER_HORARIO_SUCURSAL
...
def controlador_py_logger_get_by_id_horario_sucursal(horario_id: int) -> HorarioSucursal:
    """
    Obtiene un HorarioSucursal por su ID.
    """
    try:
        horario = HorarioSucursalRepository.get_by_id(horario_id)
...
def controlador_py_logger_get_by_sucursal(sucursal_id: int) -> List[HorarioSucursal]:
    """
...
def controlador_py_logger_get_by_dia(dia_id: int) -> List[HorarioSucursal]:
    ...
    """
    try:
        horarios = HorarioSucursalRepository.get_by_dia(dia_id)
    except Exception as error:
        logger.error("Error al obtener HorarioSucursal para día %s: %s", dia_id, error)
        raise HTTPException(status_code=500, detail="Error interno del servidor") from error
    return horarios

def controlador_py_logger_create_horario_sucursal(horario: HorarioSucursal) -> HorarioSucursal:
    """
    Crea un nuevo HorarioSucursal en la base de datos.
    """
    try:
        nuevo = HorarioSucursalRepository.create(horario)
...
def controlador_py_logger_update_horario_sucursal(horario: HorarioSucursal) -> HorarioSucursal:
    """
    Actualiza un HorarioSucursal existente.
    Retorna el objeto actualizado o levanta error si no existe.
    """
    try:
        existente = HorarioSucursalRepository.get_by_id(horario.id)
        if not existente:
            logger.warning("HorarioSucursal no encontrado para actualizar con id %s", horario.id)
            raise HTTPException(status_code=404, detail="HorarioSucursal no encontrado")
        
        actualizado = HorarioSucursalRepository.update(horario)
    except Exception as error:
...
def controlador_py_logger_delete_horario_sucursal(horario_id: int) -> bool:
    """
    Elimina un HorarioSucursal por su ID.
    """
    try:
        eliminado = HorarioSucursalRepository.delete(horario_id)
    except Exception as error:
