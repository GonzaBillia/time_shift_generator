...
def get_dia_by_id(dia_id: int):
    """
    Endpoint para obtener un Día por su ID.
    """
    try:
        dia = controlador_py_logger_get_by_id_dia(dia_id)
...
def get_dia_by_nombre(nombre: str):
    """
    Endpoint para obtener un Día por su nombre.
    """
    try:
        dia = controlador_py_logger_get_by_nombre_dia(nombre)
        dia_schema = DiaResponse.model_validate(dia)
        return success_response("Día encontrado", data=dia_schema.model_dump())
    except HTTPException as he:
        raise he
...
def get_all_dias():
    ...
    """
    try:
        dias = controlador_py_logger_get_all_dias()
...
def create_dia(dia_data: DiaBase):
    """
    Endpoint para crear un nuevo Día.
    """
    try:
        nueva_dia = Dia(**dia_data.model_dump())
        dia_creado = controlador_py_logger_create_dia(nueva_dia)
...
def update_dia(dia_id: int, dia_data: DiaBase):
    """
    Endpoint para actualizar un Día existente.
    """
    try:
        update_data = dia_data.model_dump()
        update_data["id"] = dia_id
        dia_to_update = Dia(**update_data)
        dia_actualizado = controlador_py_logger_update_dia(dia_to_update)
...
def delete_dia(dia_id: int):
    """
    Endpoint para eliminar un Día por su ID.
    """
    try:
        resultado = controlador_py_logger_delete_dia(dia_id)
