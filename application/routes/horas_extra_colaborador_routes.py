# ROUTER_PY_SCHEMA_HORAS_EXTRA_COLABORADOR
...
def get_horas_extra_by_colaborador(colaborador_id: int):
    """
    Endpoint para obtener todas las horas extra de un colaborador.
    """
    try:
        registros = controlador_py_logger_get_by_colaborador_horas_extra(colaborador_id)
...
def get_horas_extra_by_tipo(colaborador_id: int, tipo: str):
    """
    Endpoint para obtener las horas extra de un colaborador filtradas por tipo.
    """
    try:
        registros = controlador_py_logger_get_by_tipo_horas_extra(colaborador_id, tipo)
        registros_schema = [HorasExtraColaboradorResponse.model_validate(r) for r in registros]
        data = [jsonable_encoder(rs.model_dump()) for rs in registros_schema]
...
def create_horas_extra(horas_extra_data: HorasExtraColaboradorBase):
    """
    Endpoint para crear un nuevo registro de horas extra.
    """
    try:
        nuevo_registro = HorasExtraColaborador(**horas_extra_data.model_dump())
        creado = controlador_py_logger_create_horas_extra(nuevo_registro)
...
def delete_horas_extra(horas_extra_id: int):
    """
    Endpoint para eliminar un registro de horas extra por su ID.
    """
    try:
        resultado = controlador_py_logger_delete_horas_extra(horas_extra_id)
